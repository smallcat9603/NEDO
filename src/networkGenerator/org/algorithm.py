...
def get_slot_num(edges, src_dst, src_dst_sw):
    slot_num = {} 
    total = 0
    max_hops = 0 #huyao 171011
    #print "routes:",
    routes = {}
    for i in range(len(src_dst_sw)):   
        if src_dst_sw[i][0] != src_dst_sw[i][1]: 
            length, Shortest_path = dijkstra(edges, src_dst_sw[i][0], src_dst_sw[i][1])
            total += length
            if length > max_hops:
                max_hops = length
            #print str(i), ": ", str(10000+src_dst[i][0]), Shortest_path,  str(10000+src_dst[i][1]), ", ",
            routes[i] = []
            ...
            if (src_dst_sw[i][1], 10000+src_dst[i][1]) not in slot_num:
                ...
                slot_num[(src_dst_sw[i][1], 10000+src_dst[i][1])] = slot_num[(src_dst_sw[i][1], 10000+src_dst[i][1])] + 1 
            #print str(i), ": ", str(10000+src_dst[i][0]), " ", str(src_dst_sw[i][0]), " ",  str(10000+src_dst[i][1]), ", ",
    #print ""
            ...
            for df in dataflow:
                for i in range(len(routes[df])-2):
                    if routes[df][i] == pair[0]: 
                        if routes[df][i+1] in sw_sw[pair[1]] and ((pair[1], routes[df][i+1]) not in slot_num.keys() or slot_num[(pair[1], routes[df][i+1])] < slots - 1): #indirect path  pair[0]->routes[df][i+1] to pair[0]->pair[1]->routes[df][i+1]
                        ...
                        elif routes[df][i+2] in sw_sw[pair[1]] and routes[df][i+1] != pair[1] and ((pair[1], routes[df][i+2]) not in slot_num.keys() or slot_num[(pair[1], routes[df][i+2])] < slots - 1): #parallel path  pair[0]->routes[df][i+1]->routes[df][i+2] to pair[0]->pair[1]->routes[df][i+2]
                            if (pair[0], pair[1]) not in slot_num:
                                slot_num[(pair[0], pair[1])] = 1
                            else:
                                slot_num[(pair[0], pair[1])] = slot_num[(pair[0], pair[1])] + 1  
                        ...
                        if routes[df][i+1] in sw_sw[pair[0]] and ((routes[df][i+1], pair[0]) not in slot_num.keys() or slot_num[(routes[df][i+1], pair[0])] < slots - 1): #indirect path  routes[df][i+1]->pair[1] to routes[df][i+1]->pair[0]->pair[1]
                            if (pair[0], pair[1]) not in slot_num:
                                slot_num[(pair[0], pair[1])] = 1
                            else:
                                slot_num[(pair[0], pair[1])] = slot_num[(pair[0], pair[1])] + 1  
                        ...
                        elif routes[df][i] in sw_sw[pair[0]] and routes[df][i+1] != pair[0] and ((routes[df][i], pair[0]) not in slot_num.keys() or slot_num[(routes[df][i], pair[0])] < slots - 1): #parallel path  routes[df][i]->routes[df][i+1]->pair[1] to routes[df][i]->pair[0]->pair[1]
                            if (pair[0], pair[1]) not in slot_num:
                                slot_num[(pair[0], pair[1])] = 1
                            else:
                                slot_num[(pair[0], pair[1])] = slot_num[(pair[0], pair[1])] + 1  
                    ...
                    elif routes[df][i] in sw_sw[pair[0]] and routes[df][i] != pair[1] and routes[df][i+1] in sw_sw[pair[1]] and routes[df][i+1] != pair[0] and ((routes[df][i], pair[0]) not in slot_num.keys() or slot_num[(routes[df][i], pair[0])] < slots - 1) and ((pair[1], routes[df][i+1]) not in slot_num.keys() or slot_num[(pair[1], routes[df][i+1])] < slots - 1): #indirect path  routes[df][i]->routes[df][i+1] to routes[df][i]->pair[0]->pair[1]->routes[df][i+1]
                        if (pair[0], pair[1]) not in slot_num:
                            slot_num[(pair[0], pair[1])] = 1
                        else:
                            slot_num[(pair[0], pair[1])] = slot_num[(pair[0], pair[1])] + 1  
                    ...
                    elif routes[df][i] in sw_sw[pair[0]] and routes[df][i] != pair[1] and routes[df][i+2] in sw_sw[pair[1]] and routes[df][i+2] != pair[0] and routes[df][i+1] != pair[0] and routes[df][i+1] != pair[1] and ((routes[df][i], pair[0]) not in slot_num.keys() or slot_num[(routes[df][i], pair[0])] < slots - 1) and ((pair[1], routes[df][i+1]) not in slot_num.keys() or slot_num[(pair[1], routes[df][i+1])] < slots - 1): #indirect path  routes[df][i]->routes[df][i+1]->routes[df][i+2] to routes[df][i]->pair[0]->pair[1]->routes[df][i+2]
                        if (pair[0], pair[1]) not in slot_num:
                            slot_num[(pair[0], pair[1])] = 1
                        else:
                            slot_num[(pair[0], pair[1])] = slot_num[(pair[0], pair[1])] + 1  
                        ...
                        routes[df].insert(i+1, pair[0])      
                        routes[df].insert(i+2, pair[1])                                                  
                    elif i+3<len(routes[df]) and routes[df][i] in sw_sw[pair[0]] and routes[df][i] != pair[1] and routes[df][i+3] in sw_sw[pair[1]] and routes[df][i+3] != pair[0] and routes[df][i+1] != pair[0] and routes[df][i+1] != pair[1] and routes[df][i+2] != pair[0] and routes[df][i+2] != pair[1] and ((routes[df][i], pair[0]) not in slot_num.keys() or slot_num[(routes[df][i], pair[0])] < slots - 1) and ((pair[1], routes[df][i+1]) not in slot_num.keys() or slot_num[(pair[1], routes[df][i+1])] < slots - 1): #parallel path  routes[df][i]->routes[df][i+1]->routes[df][i+2]->routes[df][i+3] to routes[df][i]->pair[0]->pair[1]->routes[df][i+3]
                        if (pair[0], pair[1]) not in slot_num:
                            slot_num[(pair[0], pair[1])] = 1
                        else:
                            slot_num[(pair[0], pair[1])] = slot_num[(pair[0], pair[1])] + 1  
            ...
            for df in dataflow:
                for i in range(len(routes[df])-2):
                    if routes[df][i] == pair[1]: 
                        if routes[df][i+1] in sw_sw[pair[0]] and ((pair[0], routes[df][i+1]) not in slot_num.keys() or slot_num[(pair[0], routes[df][i+1])] < slots - 1): #indirect path  pair[1]->routes[df][i+1] to pair[1]->pair[0]->routes[df][i+1]
                            ...
                            total = total + 1
                            routes[df].insert(i+1, pair[0])                                     
                        elif routes[df][i+2] in sw_sw[pair[0]] and routes[df][i+1] != pair[0] and ((pair[0], routes[df][i+2]) not in slot_num.keys() or slot_num[(pair[0], routes[df][i+2])] < slots - 1): #parallel path  pair[1]->routes[df][i+1]->routes[df][i+2] to pair[1]->pair[0]->routes[df][i+2]
                            if (pair[1], pair[0]) not in slot_num:
                                slot_num[(pair[1], pair[0])] = 1
                            else:
                                slot_num[(pair[1], pair[0])] = slot_num[(pair[1], pair[0])] + 1  
                        ...
                        if routes[df][i+1] in sw_sw[pair[1]] and ((routes[df][i+1], pair[1]) not in slot_num.keys() or slot_num[(routes[df][i+1], pair[1])] < slots - 1): #indirect path  routes[df][i+1]->pair[0] to routes[df][i+1]->pair[1]->pair[0]
                            if (pair[1], pair[0]) not in slot_num:
                                slot_num[(pair[1], pair[0])] = 1
                            else:
                                slot_num[(pair[1], pair[0])] = slot_num[(pair[1], pair[0])] + 1  
                            ...
                            total = total + 1
                            routes[df].insert(i+2, pair[1])                                     
                        elif routes[df][i] in sw_sw[pair[1]] and routes[df][i+1] != pair[1] and ((routes[df][i], pair[1]) not in slot_num.keys() or slot_num[(routes[df][i], pair[1])] < slots - 1): #parallel path  routes[df][i]->routes[df][i+1]->pair[0] to routes[df][i]->pair[1]->pair[0]
                            if (pair[1], pair[0]) not in slot_num:
                                slot_num[(pair[1], pair[0])] = 1
                            else:
                                slot_num[(pair[1], pair[0])] = slot_num[(pair[1], pair[0])] + 1  
                            ...
                            routes[df].pop(i+1)
                            routes[df].insert(i+1, pair[1])               
                    elif routes[df][i] in sw_sw[pair[1]] and routes[df][i] != pair[0] and routes[df][i+1] in sw_sw[pair[0]] and routes[df][i+1] != pair[1] and ((routes[df][i], pair[1]) not in slot_num.keys() or slot_num[(routes[df][i], pair[1])] < slots - 1) and ((pair[0], routes[df][i+1]) not in slot_num.keys() or slot_num[(pair[0], routes[df][i+1])] < slots - 1): #indirect path  routes[df][i]->routes[df][i+1] to routes[df][i]->pair[1]->pair[0]->routes[df][i+1]
                        if (pair[1], pair[0]) not in slot_num:
                            slot_num[(pair[1], pair[0])] = 1
                        else:
                            slot_num[(pair[1], pair[0])] = slot_num[(pair[1], pair[0])] + 1  
                        ...
                        routes[df].insert(i+1, pair[1])      
                        routes[df].insert(i+2, pair[0])  
                    elif routes[df][i] in sw_sw[pair[1]] and routes[df][i] != pair[0] and routes[df][i+2] in sw_sw[pair[0]] and routes[df][i+2] != pair[1] and routes[df][i+1] != pair[1] and routes[df][i+1] != pair[0] and ((routes[df][i], pair[1]) not in slot_num.keys() or slot_num[(routes[df][i], pair[1])] < slots - 1) and ((pair[0], routes[df][i+1]) not in slot_num.keys() or slot_num[(pair[0], routes[df][i+1])] < slots - 1): #indirect path  routes[df][i]->routes[df][i+1]->routes[df][i+2] to routes[df][i]->pair[1]->pair[0]->routes[df][i+2]
                        if (pair[1], pair[0]) not in slot_num:
                            slot_num[(pair[1], pair[0])] = 1
                        else:
                            slot_num[(pair[1], pair[0])] = slot_num[(pair[1], pair[0])] + 1  
                        ...
                        routes[df].insert(i+1, pair[1])      
                        routes[df].insert(i+2, pair[0])                                                  
                    elif i+3<len(routes[df]) and routes[df][i] in sw_sw[pair[1]] and routes[df][i] != pair[0] and routes[df][i+3] in sw_sw[pair[0]] and routes[df][i+3] != pair[1] and routes[df][i+1] != pair[1] and routes[df][i+1] != pair[0] and routes[df][i+2] != pair[1] and routes[df][i+2] != pair[0] and ((routes[df][i], pair[1]) not in slot_num.keys() or slot_num[(routes[df][i], pair[1])] < slots - 1) and ((pair[0], routes[df][i+1]) not in slot_num.keys() or slot_num[(pair[0], routes[df][i+1])] < slots - 1): #parallel path  routes[df][i]->routes[df][i+1]->routes[df][i+2]->routes[df][i+3] to routes[df][i]->pair[1]->pair[0]->routes[df][i+3]
                        if (pair[1], pair[0]) not in slot_num:
                            slot_num[(pair[1], pair[0])] = 1
                        else:
                            slot_num[(pair[1], pair[0])] = slot_num[(pair[1], pair[0])] + 1  
